  ...
  if (vertical) {
    ...
    return (
      <div className="leaderboard-section">
        <h3>{title}</h3>
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-end', gap: '1.5rem', marginTop: '2rem' }}>
          {/* Bars */}
          {sortedEntries.map((entry, idx) => (
            <div className="leaderboard-bar-vertical-item" key={entry.name} style={{ alignItems: 'flex-end', width: '60px' }}>
          ...
          {/* Names in a column */}
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', height: '140px', marginLeft: '1.5rem', gap: '1.5rem' }}>
            {sortedEntries.map((entry, idx) => (
